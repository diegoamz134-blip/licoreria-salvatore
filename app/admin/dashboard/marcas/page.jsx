...
export default function MarcasPage() {
  ...
  const load = async () => {
    setLoading(true);
    // Usamos el endpoint genérico o uno específico si decides crearlo. 
    // Por ahora, consultaremos directamente a través de una API interna o el cliente de Supabase.
    // Para mantener consistencia con categorías, asumo que usaremos /api/admin/brands
    try {
  ...
  const handleAdd = async (e) => {
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      setError(data.error?.includes('unique') ? 'Esta marca ya existe.' : (data.error || 'Error al agregar'));
    }
    setSaving(false);
  };
        ...
        <div className="bg-[#141414] border border-white/[0.06] p-5">
          ...
          <form onSubmit={handleAdd} className="flex gap-2">
            <input
              ...
              placeholder="Ej: Johnnie Walker, Cartavio, Queirolo…"
